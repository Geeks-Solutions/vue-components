<template>
  <div ref="myQuillEditorRef" title="" class="quill-editor wyzywig input wyzywig-wrapper">
    ...
    <LazyGMediaComponent
      ...
      @emitted-media="handleEmittedMedia"
    />
  </div>
</template>
...
const htmlEditorTextareaElement = null
...
const defineQuillModules = async () => {
  ...
  class ButtonBlot extends Embed {
    ...
    static create(value) {
      ...
      buttonContainer.setAttribute(
        'data-button-id',
        'button-' + Date.now() + '-' + Math.floor(Math.random() * 1000)
      )
  ...
  class ButtonToolbarModule {
    constructor(q, opts) {
      this.quill = q
      this.options = opts
      this.toolbar = q.getModule('toolbar')
    ...
    handleButtonAction() {
      if (this.selectedButtonId) {
        const btnEl = this.quill.root.querySelector(
          `.quill-button-container[data-button-id="${this.selectedButtonId}"]`
        )
        if (btnEl) {
          this.editButton(btnEl)
          return
        }
      }
      this.createButton()
...
const initEditorOptions = () => {
  ...
  if (
    ...
    props.sectionsWysiwygEditorOptions &&
    Object.keys(props.sectionsWysiwygEditorOptions).length > 0 &&
    props.sectionsWysiwygEditorOptions.modules
  ) {
...
watch(selectedMedia, async (mediaObject) => {
  ...
  media.url =
    mediaObject.url || (mediaObject.files && mediaObject.files[0] ? mediaObject.files[0].url : '')
...
async function handleEmittedMedia(media) {
  if (
    injectedTransformer &&
    typeof injectedTransformer === 'function' &&
    media.files &&
...
function applyFormat() {
  ...
  if (quill && savedFormatJson) {
    ...
    if (selection) {
      try {
        const savedFormat =
          typeof savedFormatJson === 'string' ? JSON.parse(savedFormatJson) : savedFormatJson
...
const onQuillEditorReady = async (quillInstance) => {
  ...
  if (editorElement) {
    editorElement.querySelectorAll('.ql-save-format').forEach((btn) => {
      btn.innerHTML =
        '<div title="Save format"><svg version="1.0" style="width: 20px;" xmlns="http://www.w3.org/2000/svg"\n' +
    ...
    editorElement.querySelectorAll('.ql-apply-format').forEach((btn) => {
      btn.innerHTML =
        '<div title="Apply format"><svg version="1.1" id="Layer_1" style="width: 20px;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"\n' +
    ...
    editorElement.querySelectorAll('.ql-button').forEach((btn) => {
      btn.innerHTML =
        '<div title="Add/Edit a button"><svg viewBox="0 0 18 18"><rect width="16" height="10" x="1" y="4" rx="2" ry="2" stroke-width="1.5" stroke="currentColor" fill="none"></rect><path d="M5,9 L13,9" stroke-width="1.5" stroke="currentColor"></path></svg></div>'
    })
    ...
    if (additionalButton) {
      ...
      additionalButton.addEventListener('click', (e) => {
        ...
        formatGroups.forEach((el) => {
          const isAdditional = el.querySelector(
            'button.ql-formula, button.ql-blockquote, button.ql-code-block, button.ql-script, button.ql-direction, button.ql-emoji, button.ql-button, button.ql-html, button.ql-table-button'
          )
          if (isAdditional) {
...
const appendEmptyParagraphIfOnlyRawHtmlContainer = (htmlContent) => {
  try {
    ...
    const isQLOne = (el) => el.tagName === 'DIV' && el.classList.contains('ql-custom-html')

    const len = bodyChildren.length

    if (len === 1 && isQLOne(bodyChildren[0])) {
      // Only element
      htmlContent += '<p>&nbsp;</p>'
    } else if (len >= 1 && isQLOne(bodyChildren[len - 1])) {
      // Last element
      htmlContent += '<p>&nbsp;</p>'
    } else if (len >= 2 && isQLOne(bodyChildren[len - 2])) {
...
<style>
.ql-toolbar.ql-snow .ql-formats button.ql-save-format,
.ql-toolbar.ql-snow .ql-formats button.ql-apply-format {
